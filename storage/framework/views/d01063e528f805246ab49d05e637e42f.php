                ...
                <?php if(count($rows)>0){ $sl=1; foreach($rows as $row){?>
                  <tr>
                    ...
                    <td>
                      ...
                      <?php }?>
                      <a href="<?=url('admin/' . $controllerRoute . '/view-details/'.Helper::encoded($row->id))?>" class="btn btn-outline-info btn-sm" title="View <?=$module['title']?>"><i class="fa fa-info-circle"></i></a>
                      <br><br>
                      <?php
                      $labelCount = StudentLabelMark::where('student_id', '=', $row->id)->count();
                      ?>
                      <a href="<?=url('admin/' . $controllerRoute . '/label-marks/'.Helper::encoded($row->id))?>" class="btn btn-primary btn-sm" title="Edit <?=$module['title']?>"><i class="fa fa-clock"></i> Label Wise Marks (<?=$labelCount?>)</a>
                    </td>
                  </tr>
                <?php } } else {?>
                  <tr>
...
</section><?php /**PATH G:\xampp8.2\htdocs\kids-zone\resources\views/admin/maincontents/student/list.blade.php ENDPATH**/ ?>