            ...
            <tbody>
              <?php if($rows){ $sl=1; foreach($rows as $row){?>
                <tr>
                  ...
                  <td><?=$row->name?></td>
                  <td><?=$row->location?></td>
                  <td><?=date_format(date_create($row->checkin_time), "h:i A")?></td>
                  <td><?=date_format(date_create($row->checkout_time), "h:i A")?></td>
                  <td>
                    <?php if($row->cover_image != ''){?>
                      <img src="<?=env('UPLOADS_URL').'hotel/'.$row->cover_image?>" class="img-thumbnail" alt="<?=$row->name?>" style="width: 150px; height: 150px; margin-top: 10px;">
                    <?php } else {?>
...
</section><?php /**PATH E:\xampp8.2\htdocs\relook\resources\views/admin/maincontents/hotel/list.blade.php ENDPATH**/ ?>